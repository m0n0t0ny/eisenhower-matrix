...
class TaskModel {
  ...
  restoreTask(taskId) {
    const taskIndex = this.completedTasks.findIndex(
      (task) => task.id === taskId
    );
...
class TaskView {
  constructor() {
    ...
    this.recentCompletedTasksContainer = document.getElementById(
      "recentCompletedTasks"
    );
    this.allCompletedTasksContainer =
      document.getElementById("allCompletedTasks");
  ...
  bindEditTask(handler) {
    for (let i = 1; i <= 4; i++) {
      ...
      this.taskLists[i].addEventListener("click", (e) => {
        ...
        if (
          e.target.classList.contains("save-edit-btn") ||
          ...
          e.target.classList.contains("cancel-edit-btn") ||
      ...
      if (e.target.closest(".restore-btn")) {
        ...
        setTimeout(() => {
          ...
          this.showToast(
            ...
            "L'attività è stata riportata nel quadrante originale."
          );
        }, 300);
      }
    });
  ...
  bindDeleteCompletedTask(handler) {
    ...
    this.allCompletedTasksContainer.addEventListener("click", (e) => {
      if (e.target.closest(".delete-btn")) {
        ...
        const confirmDelete = confirm(
          "Sei sicuro di voler eliminare definitivamente questa attività completata?"
        );
        if (confirmDelete) {
          ...
          setTimeout(() => {
            ...
            this.showToast(
              "info",
              "Attività eliminata",
              "L'attività completata è stata eliminata definitivamente."
            );
          }, 300);
        }
      }
  ...
  createTaskElement(task) {
    ...
    dragHandle.innerHTML =
      '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="8" cy="6" r="2"/><circle cx="8" cy="12" r="2"/><circle cx="8" cy="18" r="2"/><circle cx="16" cy="6" r="2"/><circle cx="16" cy="12" r="2"/><circle cx="16" cy="18" r="2"/></svg>';
    ...
    if (task.editedAt) {
      ...
      editedIndicator.title = `Modificato il ${this.formatDate(
        new Date(task.editedAt)
      )}`;
      editedIndicator.innerHTML =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 14.66V20a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h5.34"></path><polygon points="18 2 22 6 12 16 8 16 8 12 18 2"></polygon></svg>';
...
class TaskController {
  ...
  bindEvents() {
    ...
    this.view.bindMarkTaskCompleted((taskId) => {
      ...
      this.view.renderTasks(this.getTasksForAllQuadrants());
      this.view.renderRecentCompletedTasks(
        this.model.getRecentCompletedTasks()
      );
      this.view.showToast(
        "success",
        "Attività completata",
        "L'attività è stata spostata tra le completate."
      );
    });
    ...
    this.view.bindDragAndDrop((taskId, newQuadrant) => {
      ...
      this.view.showToast(
        "info",
        "Attività spostata",
        `L'attività è stata spostata nel quadrante Q${newQuadrant}.`
      );
    });
  ...
  toggleTheme() {
    ...
    if (this.currentTheme === "dark") {
      view.showToast(
        "info",
        "Tema scuro attivato",
        "Il tema scuro è stato attivato."
      );
    } else {
      view.showToast(
        "info",
        "Tema chiaro attivato",
        "Il tema chiaro è stato attivato."
      );
    }
  }
...
// Initialize theme manager when app loads
document.addEventListener("DOMContentLoaded", () => {
