...
const directories = ["icons", "screenshots"];
...
if (hasImageMagick) {
  console.log(
    "  ✅ ImageMagick trovato, procedo con la generazione delle icone."
  );
  ...
  iconSizes.forEach((size) => {
    ...
    try {
      execSync(
        `convert -background none temp-icon.svg -resize ${size}x${size} ${iconPath}`
      );
      console.log(`    ✅ Creata icona: ${iconPath}`);
    } catch (error) {
      console.log(
        `    ❌ Errore durante la generazione di ${iconPath}: ${error.message}`
      );
    }
  });
  ...
  shortcutIcons.forEach((icon) => {
    ...
    try {
      execSync(
        `convert -background none temp-icon.svg -resize 192x192 ${iconPath}`
      );
  ...
  console.log(
    "  ⚠️ ImageMagick non trovato. Le icone dovranno essere create manualmente."
  );
  console.log(
    "  ℹ️ Installa ImageMagick per generare automaticamente le icone:"
  );
  console.log("    - Su macOS: brew install imagemagick");
  console.log("    - Su Linux: sudo apt-get install imagemagick");
  console.log(
    "    - Su Windows: Scarica da https://imagemagick.org/script/download.php"
  );
...
screenshots.forEach((screenshot) => {
  ...
  if (!fs.existsSync(screenshotPath)) {
    ...
    console.log(
      `     Dovrebbe essere di dimensioni ${screenshot.width}x${screenshot.height}`
    );

    if (hasImageMagick) {
      try {
        execSync(
          `convert -size ${screenshot.width}x${screenshot.height} canvas:white -fill "#4a6fa5" -gravity center -pointsize 30 -annotate 0 "Screenshot ${screenshot.name}" ${screenshotPath}`
        );
        console.log(`    ✅ Creato screenshot di esempio: ${screenshotPath}`);
      } catch (error) {
        console.log(
          `    ❌ Errore durante la generazione di ${screenshotPath}: ${error.message}`
        );
      }
    }
  } else {
...
try {
  ...
  console.log(
    "  ❌ Errore nella lettura o parsing di manifest.json:",
    error.message
  );
}
...
try {
  ...
  const missingCachedResources = essentialCachedResources.filter(
    (resource) => !swContent.includes(resource)
  );

  if (missingCachedResources.length > 0) {
    console.log(
      "  ⚠️ Alcune risorse essenziali potrebbero non essere cachate nel Service Worker:"
    );
    missingCachedResources.forEach((resource) =>
      console.log(`    - ${resource}`)
    );
  } else {
    console.log(
      "  ✅ Il Service Worker sembra configurato per cachare le risorse essenziali."
    );
  }
} catch (error) {
...
console.log(
  '  3. Nella scheda "Applicazione" degli strumenti di sviluppo, verifica:'
);
...
console.log(
  "  4. Prova ad installare l'app usando il pulsante \"Installa\" o l'opzione del browser"
);
console.log(
  "  5. Testa la funzionalità offline disabilitando la rete negli strumenti di sviluppo\n"
);
